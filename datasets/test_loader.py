...
from utils import hwc_to_chw, test_read_img
...
class PairLoader(Dataset):
    def __init__(self, data_dir, sub_dir, mode, size=256, edge_decay=0, only_h_flip=False):
        ...
        self.img_names = sorted(os.listdir(os.path.join(self.root_dir, 'high')))
    ...
    def __getitem__(self, idx):
        ...
        source_img = test_read_img(os.path.join(self.root_dir, 'low', img_name))
        target_img = test_read_img(os.path.join(self.root_dir, 'high', img_name))
...
class SingleLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # read image, and scale [0, 1] to [-1, 1]
        img_name = self.img_names[idx]
        img = test_read_img(os.path.join(self.root_dir, img_name))
