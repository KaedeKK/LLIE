...
class Channel_Embeddings(nn.Module):
    ...
    """
    def __init__(self,config, patchsize, img_size, in_channels,B):
        super().__init__()
        img_size = _pair(img_size)
        patch_size = _pair(patchsize)
        # print(img_size)
        n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])
        # print(n_patches,patch_size)
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       out_channels=in_channels,
                                       kernel_size=patch_size,
                                       stride=patch_size,
                                       )
        # self.position_embeddings = nn.Parameter(torch.zeros(B, n_patches, in_channels))
    ...
    def forward(self, x):
        ...
        h_size,w_size = x.size()[2],x.size()[3]
        x = x.flatten(2)
        x = x.transpose(-1, -2)  # (B, n_patches, hidden)
        embeddings = x
        embeddings = self.dropout(embeddings)
        return embeddings,h_size,w_size
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num, patchSize,Batch=16):
        super().__init__()
        ...
        # print('img',img_size,patchSize)
        self.embeddings_1 = Channel_Embeddings(config,self.patchSize_1, img_size=img_size[0],    in_channels=channel_num[0],B=Batch)
        self.embeddings_2 = Channel_Embeddings(config,self.patchSize_2, img_size=img_size[1], in_channels=channel_num[1],B=Batch)
        self.embeddings_3 = Channel_Embeddings(config,self.patchSize_3, img_size=img_size[2], in_channels=channel_num[2],B=Batch)
    ...
    def forward(self,en1,en2,en3):
        ...
        emb1,h1,w1 = self.embeddings_1(en1)
        emb2,h2,w2 = self.embeddings_2(en2)
        emb3,h3,w3 = self.embeddings_3(en3)
        # print(emb1.size(),emb2.size())
        # emb4 = self.embeddings_4(en4)

        encoded1, encoded2, encoded3 = self.encoder(emb1,emb2,emb3)  # (B, n_patch, hidden)
        # print(encoded1.size(),encoded2.size())
        x1 = self.reconstruct_1(encoded1,h1,w1) if en1 is not None else None
